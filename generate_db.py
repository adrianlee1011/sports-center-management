...
f_description = ["""This is a stae of the art swimming pool with multiple lanes for different speed swimmers. It can be booked for General Use, Lane Swimming,  Lessons, or Team Events (£2 per session)""",
        """Our newly refurbished fitness room contains a range of equipment including free weights, treadmills and rowing machines. It can only be booked for General Use (£2 per session)""",
        """This is one of our four squash courts that can be booked for 1-hour sessions as well as team events (£5 per session)""",
        """This is one of our four squash courts that can be booked for 1-hour sessions as well as team events (£5 per session)""",
        """This is one of our four squash courts that can be booked for 1-hour sessions as well as team events (£5 per session)""",
        """This is one of our four squash courts that can be booked for 1-hour sessions as well as team events (£5 per session)""",
        """Our large Sports Hall can be used to play a variety of games and can be booked for 1-hour sessions (£2 per session)""",
        ]
...
for i in range(len(u_names)):
  ...
  u = models.User(id=i+2, name=u_names[i], email=u_email.lower(), password=u_password, user_type=1, card_number="none", card_expiry="none", card_CVC="none")
  db.session.add(u)
  db.session.commit()
  if i%20 == 0:
